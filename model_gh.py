...
def print_softmax_stats():
    ...
    global softmax_total_latency, softmax_total_count
    avg_latency = softmax_total_latency / softmax_total_count if softmax_total_count > 0 else 0
    print("\n🔥 Softmax Performance Stats 🔥")
    print(f"📌 Total Softmax Latency: {softmax_total_latency} seconds")
    print(f"📌 Softmax Execution Count: {softmax_total_count}")
    print(f"📌 Average Softmax Latency: {avg_latency} seconds")
    print(f"📌 Total GPT Execution Count: {gpt_count}\n")
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, iter_num):
        ...
        if self.flash:
            ...
            ## print(att.shape)        ## torch.Size([1, 6, 256, 256])
            # softmax variationc
            start_time = time.perf_counter()
            if self.softmax_variant_attn != 'softmax':
