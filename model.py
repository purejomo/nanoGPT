...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, iter_num, layer_idx=None):
        ...
        if self.flash:
            ...
            if self.window_size is not None:
                ...
                att = att.masked_fill(self.bias[:,:,:T,:T].to(x.device) == 0, float('-inf'))        ## GH
                ## att = att.masked_fill(self.bias[:,:,:T,:T].to(x.device) == 0, -6e4)
            ...
            ## latency measurement for Summer Annual Conference of IEIE
            # L = 1024
            # att_small = att[:, :, :L, :L]
            # # softmax variation   
            # if self.softmax_variant_attn != 'softmax':
            #     att_small = self.softmax_layer_attn(att_small)
            # else:
            #     softmaxPV_start_time = time.perf_counter()
            #     m = att_small.max(dim=-1, keepdim=True)[0]                       # (B, nh, T, 1)
            #     att_stable = att_small - m                                       # broadcast subtraction
            #     exp_att = torch.exp(att_stable)                                     # exponentiation
            #     att_small = exp_att / exp_att.sum(dim=-1, keepdim=True)          # normalize        
            ...
            if self.softmax_variant_attn != 'softmax':
                ...
                ## att = torch.exp(att) / torch.sum(torch.exp(att), dim=-1, keepdim=True)            
            ## softmaxPV_end_time = time.perf_counter()
            ## print(f"softmaPV time: {(softmaxPV_end_time - softmaxPV_start_time) * 1000000}")
            
            if not os.path.exists("attention_heatmaps"):
                os.makedirs("attention_heatmaps")
            B, nh, T, _ = att.shape
            for b in range(B):
                for h in range(nh):
                    att_matrix = att[b, h].detach().cpu().numpy()  # (T, T) shape
                    plt.figure(figsize=(6, 6))
                    ## plt.imshow(att_matrix, cmap='gray')
                    plt.imshow(att_matrix, cmap='viridis')
                    plt.xlabel('Key')
                    plt.ylabel('Query')
                    plt.title(f'Attention Heatmap (Batch {b}, Layer {layer_idx}, Head {h})')
                    plt.colorbar()
                    plt.savefig(f"attention_heatmaps/att_heatmap_b{b}_layer{layer_idx}_h{h}.png")
                    plt.close()
        ...
        ## print(f"attention time: {(attn_end_time - attn_start_time) * 1000000}")
        return y
...
class Block(nn.Module):
    def __init__(self, config, mlp=None, attn=None, layer_idx=None):
        super().__init__()
        ...
        def custom_forward(*inputs):
            ...
            if self.use_post_ln:
                if self.use_parallel_mlp:
                    x = self.ln_1(x + self.attn(x, iter_num, layer_idx=self.layer_idx) + self.mlp(x, iter_num))
                else:
                    x = self.ln_1(x + self.attn(x, iter_num, layer_idx=self.layer_idx))
                    x = self.ln_2(x + self.mlp(x, iter_num))
            else:
                if self.use_parallel_mlp:
                    ln_1 = self.ln_1(x)
                    x = x + self.attn(ln_1, iter_num, layer_idx=self.layer_idx) + self.mlp(ln_1, iter_num)
                else:
                    x = (x + self.attn(self.ln_1(x), iter_num, layer_idx=self.layer_idx))
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            ## h = nn.ModuleList([Block(config, mlp=shared_mlp_array[i], attn=shared_attn_array[i]) for i in range(config.n_layer)]), ##GH
            h = nn.ModuleList([Block(config, mlp=shared_mlp_array[i], attn=shared_attn_array[i], layer_idx=i) for i in range(config.n_layer)]),
...
# def calculate_sparsity(att):
#     """어텐션 출력 벡터에서의 sparsity(0 값 비율) 계산"""
#     total_elements = att.numel()
#     zero_elements = torch.sum(att == 0).item()
#     sparsity = (zero_elements / total_elements) * 100
#     sparsity_list.append(sparsity)
#     return sparsity

# # 기존에 있던 sparsity_list처럼, q/k/v/QK/SV/Proj별로 따로 리스트를 둠
# sparsity_q_list = []
# sparsity_k_list = []
# sparsity_v_list = []
# sparsity_qk_list = []
# sparsity_sv_list = []
# sparsity_proj_list = []
# def save_zero_indices_to_file(tensor, output_file_path, threshold=0.0):
#     # 디렉토리 생성
...
#     """ qkv 각각에 대해 누적된 sparsity의 평균을 출력 """
#     # qkv
#     if sparsity_q_list:
#         avg_q = sum(sparsity_q_list) / len(sparsity_q_list)
#         print(f"🔥 평균 q sparsity: {avg_q:.12f}% (총 {len(sparsity_q_list)}회)")
#     else:
#         print("⚠️ 저장된 q sparsity 데이터가 없습니다.")

#     if sparsity_k_list:
#         avg_k = sum(sparsity_k_list) / len(sparsity_k_list)
#         print(f"🔥 평균 k sparsity: {avg_k:.12f}% (총 {len(sparsity_k_list)}회)")
#     else:
#         print("⚠️ 저장된 k sparsity 데이터가 없습니다.")

#     if sparsity_v_list:
#         avg_v = sum(sparsity_v_list) / len(sparsity_v_list)
#         print(f"🔥 평균 v sparsity: {avg_v:.12f}% (총 {len(sparsity_v_list)}회)")
#     else:
#         print("⚠️ 저장된 v sparsity 데이터가 없습니다.")
...
#     def forward(self, x, iter_num):
#         # Global variables to track attention latency
#         global attention_total_latency, attention_total_count
#         global tmp
#         B, T, C = x.size() # batch size, sequence length, embedding dimensionality (n_embd)              # x: [B, 256, embed_dim]        torch.Size([1, 256, 384]) fix
#         ## print(f"Shape of the x tensor :  {x.shape}")
#         tmp += 1
#         ## iter_num = NONE
#         if tmp < 24:
#             zero_count = (x == 0).sum().item()
#             ## print(f"⚠️처음 x 0값 개수: {zero_count} / {x.numel()} ({zero_count / x.numel() * 100:.2f}%)")
...
#         q_sp=calculate_sparsity_anyshape(q)
#         sparsity_q_list.append(q_sp)
#         k_sp=calculate_sparsity_anyshape(k)
#         sparsity_k_list.append(k_sp)
#         v_sp=calculate_sparsity_anyshape(v)
#         sparsity_v_list.append(v_sp)

#         ## update_global_min_max(q, "Q")
#         ## update_global_min_max(k, "K")
#         ## update_global_min_max(v, "V")
...
#             ## print(f"⚠️처음 q 0값 개수: {zero_count} / {q.numel()} ({zero_count / q.numel() * 100:.2f}%)")
...
#             else:
#                 att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))
#             print(f"Shape of the att tensor :  {att.shape}")
#             ## att값중에서 0보다 작은 것 비율
#             zero_count = (att < 0).sum().item()
#             total_count = att.numel()
#             print(f"qk 중에서 0보다 작은 값 개수: {zero_count} / {total_count} ({zero_count / total_count * 100:.2f}%)")
#             qk_sparsity = calculate_sparsity_anyshape(att)
#             sparsity_qk_list.append(qk_sparsity)
#             ## update_global_min_max(attn_score, "Attention Score (Before Softmax)")
...
#             ## update_global_min_max(attn_score, "Attention Score (After Softmax)")
...
#             else:
#                 y = att @ v # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
#             ## update_global_min_max(y, "Attention Output (QK * V)")
...
#         attention_total_latency += (end_time - start_time)
#         attention_total_count += 1
#         ## update_global_min_max(y, "Projection Output")
...
#     def forward(self, idx, targets=None, iter_num=None, valid_length=None):
#         global gpt_count
#         device = idx.device
#         b, t = idx.size()
#         # assert t <= self.config.block_size, f"Cannot forward sequence of length {t}, block size is only {self.config.block_size}"
...
#             # if the sequence context is growing too long we must crop it at block_size
#             idx_cond = idx if idx.size(1) <= self.config.block_size else idx[:, -self.config.block_size:]
#             print(f"idx.size(1) = {idx.size(1)}")       ## GH
#             # forward the model to get the logits for the index in the sequence
#             logits, _ = self(idx_cond)
#             # pluck the logits at the final step and scale by desired temperature
#             logits = logits[:, -1, :] / temperature
