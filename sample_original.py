...
def save_chart(probs, idx, decode, step, out_dir, last_k_tokens, chart_type, selected_token):
    print(f"Saving chart at step {step} to {out_dir}. Probs shape: {probs.shape}")
...
#                 padded_X = torch.zeros((X.size(0), block_size), dtype=X.dtype, device=X.device)
#                 padded_X[:, :token_idx + 1] = X[:, :token_idx + 1]  # Padding to match block_size
#                 with torch.amp.autocast(device_type=device, dtype=dtype):
#                     start = time.perf_counter()
#                     logits, loss = model(padded_X, Y)
...
#     print(f"Average Token-wise Latency: {avg_latency} seconds")
